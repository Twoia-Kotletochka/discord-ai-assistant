...
`VOICE_TEXT_OUTPUT_MODE=thread` отправляет текстовые копии voice-запросов и ответов в приватный thread внутри серверного текстового канала. Это не ЛС: thread находится в Discord-сервере, но виден только добавленным участникам и модераторам с правами доступа к приватным threads. Для этого боту нужны права `Create Private Threads` и `Send Messages in Threads`, либо `Administrator`. Для ЛС-режима поставьте `VOICE_TEXT_OUTPUT_MODE=dm`, для старого публичного режима - `VOICE_TEXT_OUTPUT_MODE=channel`, для полного отключения текста - `VOICE_TEXT_OUTPUT_MODE=off`. То же самое можно переключить в панели: **Управление -> Voice-поведение -> Текст voice-ответов**.
...
Плеер использует свой поток через `yt-dlp` и `ffmpeg`, а не команды других Discord-ботов. Пока играет музыка, голосовой TTS-ответ ассистента не перебивает трек: текстовая копия ответа уходит в выбранный `VOICE_TEXT_OUTPUT_MODE`, по умолчанию в приватный thread вызывающего пользователя.
