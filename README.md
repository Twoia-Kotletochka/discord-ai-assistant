...
`VOICE_TEXT_OUTPUT_MODE=dm` отправляет текстовые копии voice-запросов и ответов личным сообщением тому, кто вызвал бота. В общий канал они не пишутся. Если у пользователя закрыты DM от участников сервера, приватное сообщение может не дойти. Для старого публичного режима поставьте `VOICE_TEXT_OUTPUT_MODE=channel`, для полного отключения текста - `VOICE_TEXT_OUTPUT_MODE=off`. То же самое можно переключить в панели: **Управление -> Voice-поведение -> Текст voice-ответов**.
...
Плеер использует свой поток через `yt-dlp` и `ffmpeg`, а не команды других Discord-ботов. Пока играет музыка, голосовой TTS-ответ ассистента не перебивает трек: текстовая копия ответа уходит в DM вызывающему пользователю, если включен `VOICE_TEXT_OUTPUT_MODE=dm`.
