...
Выгнать самого бота из voice можно голосом:

```text
Бот выйди из войса
Бот покинь голосовой канал
Бот отключись от voice
Бот выгони себя из войса
```
...
Плеер использует свой поток через `yt-dlp` и `ffmpeg`, а не команды других Discord-ботов. Пока играет музыка, голосовой TTS-ответ ассистента не перебивает трек: текстовая копия ответа уходит в выбранный `VOICE_TEXT_OUTPUT_MODE`, по умолчанию в один thread вызывающего пользователя внутри `#bot` или `#bot-public`.
