...
`VOICE_TEXT_OUTPUT_MODE=thread` отправляет текстовые копии voice-запросов и ответов в один приватный thread вызывающего пользователя внутри серверного текстового канала. Это не ЛС: thread находится в Discord-сервере, но виден только добавленным участникам и модераторам с правами доступа к приватным threads. Бот сначала ищет уже существующий активный или архивный thread пользователя и переиспользует его; новый создается только если старого нет. Для этого боту нужны права `Create Private Threads` и `Send Messages in Threads`, либо `Administrator`. Для ЛС-режима поставьте `VOICE_TEXT_OUTPUT_MODE=dm`, для старого публичного режима - `VOICE_TEXT_OUTPUT_MODE=channel`, для полного отключения текста - `VOICE_TEXT_OUTPUT_MODE=off`. То же самое можно переключить в панели: **Управление -> Voice-поведение -> Текст voice-ответов**.
...
VOICE_AUTO_RESUME_ENABLED=true
```

По умолчанию фиксированное автоподключение через `AUTO_JOIN_*` выключено. Чтобы бот сам заходил в конкретный канал после старта, нужно явно поставить `AUTO_JOIN_ENABLED=true` и заполнить все три ID. Если `AUTO_JOIN_ENABLED=false`, бот не использует случайно заполненные `AUTO_JOIN_*`.

`VOICE_AUTO_RESUME_ENABLED=true` включает более удобный режим: после успешного `/join` бот запоминает текущие voice/text channel в runtime-config и после перезапуска контейнера возвращается туда сам. Команда `/leave` и idle-leave с людьми отключают возврат для этой сохраненной сессии до следующего `/join`.

Если бот один в voice-канале, он остается подключенным, но не запускает STT/AI/TTS, не делает idle-chatter и не тратит Groq/Whisper токены. Обработка начинается только когда в канале есть человек и он говорит.
...
STT-модель, выбранная в панели, используется первой. Остальные Whisper-модели подключаются только как fallback, если выбранная модель недоступна или уперлась в лимиты. В панели под выбором моделей показывается фактический порядок STT.
...
Music/radio-плеер внутри Docker уже включает `yt-dlp`. По умолчанию поиск идет по YouTube:

```bash
MUSIC_DEFAULT_VOLUME=0.45
MUSIC_MAX_QUEUE=25
```
...
- **Музыка** - включение песен/радио, pause/resume/stop, громкость и очередь.
...
- `/profile` - показать твой профиль ассистента.
...
10. После этого Discord -> Telegram будет готов. Telegram -> Discord по умолчанию останется выключенным, его можно включить отдельно через `.env` или панель.

Если Discord пишет `Telegram getChat: Bad Request: chat not found`, Telegram-бот пока не видит этот чат. Для личного чата сначала отправьте Telegram-боту `/start`. Для группы добавьте бота в группу и отправьте туда любое сообщение. После этого снова выполните `/telegram_chats` и сохраняйте `chat_id` именно из выданного списка. У supergroup `chat_id` обычно начинается с `-100`.
...
Входящие команды из Telegram:

```text
/status - статус бота и Telegram bridge
/logs 20 - последние события
/reminders - активные напоминания
/voice - активные voice-каналы
/channels - список текстовых каналов
/send привет - отправить в основной Discord-чат
/send #bot привет - отправить в конкретный Discord-канал
/send bot: привет - отправить в канал по имени через двоеточие
/cmd покажи статус - выполнить команду через Discord action-parser
...
Поиск памяти работает не только по точному тексту: запрос `про сервер` может найти записи про VPS, Docker, backup, деплой или базу, а запросы вроде `что я просил вчера` используют дату создания записи.

Профиль пользователя:

```text
Бот, называй меня Илья.
Бот, мой часовой пояс Europe/Kyiv.
Бот, любимые темы Dota 2, Docker и Telegram.
Бот, стиль общения коротко и по делу.
Бот, частые задачи backup, деплой и проверка логов.
Бот, привычные команды замьютить, переместить, отправить в Telegram.
Бот, персональная заметка я обычно тестирую бота вечером.
Бот, предпочтения по шуткам жесткие, но короткие.
Бот, покажи мой профиль.
```

Профиль хранится отдельно от обычной памяти и подмешивается в ответы, приветствия и обращения по имени.
...
Бот, напомню, 7 июня поздравить Досика с днем рождения.
Бот, не забудь 7 июня поздравить Досика с днем рождения.
Бот, напоминай каждые 2 часа размяться.
Бот, покажи напоминания.
Бот, какие напоминания на сегодня есть?
Бот, скажи мои напоминания на сегодня.
Бот, покажи мои напоминания на завтра.
Бот, покажи напоминания на неделю.
Бот, удали напоминание про чай.
Бот, удали напоминание про маршрут.
Бот, удали второе напоминание.
```

Относительные слова вроде `сегодня`, `завтра`, `послезавтра` и дни недели считаются в timezone `REMINDER_TIME_ZONE`, по умолчанию `Europe/Kyiv`. Если сказана дата без времени, например `7 июня поздравить`, бот ставит напоминание на `REMINDER_DEFAULT_HOUR:REMINDER_DEFAULT_MINUTE`, по умолчанию `09:00`. В ответе бот показывает точное время с timezone, чтобы было видно, когда напоминание реально сработает.

Музыка:

```text
Бот, включи песню No root.
Бот, поставь музыку lofi radio.
Бот, найди на YouTube песню No root и включи.
Бот, поставь музыку на паузу.
Бот, продолжи музыку.
Бот, громкость музыки 50.
Бот, следующий трек.
Бот, выключи музыку.
Бот, покажи очередь музыки.
```

Плеер использует свой поток через `yt-dlp` и `ffmpeg`, а не команды других Discord-ботов. Пока играет музыка, голосовой TTS-ответ ассистента не перебивает трек: текстовая копия ответа уходит в выбранный `VOICE_TEXT_OUTPUT_MODE`, по умолчанию в один приватный thread вызывающего пользователя.
Если музыка заглушает обращение, базовые команды управления принимаются и без trigger-word: `стоп`, `хватит`, `пауза`, `останови музыку`, `выключи музыку`.
...
- `user_profiles` - структурные профили пользователей.
- `reminders` - активные напоминания.
