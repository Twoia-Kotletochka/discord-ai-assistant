...
Если заполнить все три ID, бот после старта сам зайдет в указанный voice/stage channel и будет писать статусы в указанный текстовый канал. Если оставить поля пустыми, подключение выполняется вручную через `/join`.
...
- **Управление** - имя ассистента, trigger word, интернет-поиск, idle-фразы, подтверждения действий, healthcheck.
